...
filtered_stations = data['stationBeanList'].select do |station|
    station['totalDocks'] > 40 && station['availableBikes'] > 5
  end
  sorted_stations = filtered_stations.sort_by { |station| station['availableBikes'] }.reverse
# The list should be printed in a user-friendly way (i.e. not as a hash).
puts "Stations with more than 40 total docks and more than 5 bikes currently available:"
  sorted_stations.each do |station|
    puts "#{station['stationName']} - Available Bikes: #{station['availableBikes']}"
  end
else
